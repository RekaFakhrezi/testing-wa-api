    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm text-slate-600">
          <thead className="bg-slate-50/80 border-b border-slate-200 text-xs uppercase text-slate-500 font-semibold">
            <tr>
              ...
              <th className="px-5 py-3 cursor-pointer group">
                <div className="flex items-center gap-1">Type <SortIcon /></div>
              </th>
              <th className="px-5 py-3 cursor-pointer group">
                <div className="flex items-center gap-1">Agents <SortIcon /></div>
              </th>
              <th className="px-5 py-3 cursor-pointer group">
                <div className="flex items-center gap-1">Email Address <SortIcon /></div>
              </th>
              <th className="px-5 py-3 cursor-pointer group">
                <div className="flex items-center gap-1">Manager <SortIcon /></div>
              </th>
            </tr>
          </thead>
              ...
              <tr key={dept.id} className="hover:bg-slate-50/80 transition-colors">
                ...
                <td className="px-5 py-3.5">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                    dept.status === 'Active' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'
                  }`}>
                    {dept.status}
                  </span>
                </td>
                <td className="px-5 py-3.5 text-slate-700">{dept.type}</td>
                <td className="px-5 py-3.5 font-bold text-blue-600">{dept.agents}</td>
                <td className="px-5 py-3.5 text-slate-500">{dept.emailAddress}</td>
                <td className="px-5 py-3.5 text-slate-500">{dept.manager}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="p-4 border-t border-slate-200 text-xs font-semibold text-slate-500 flex items-center gap-2">
          <span>Select:</span>
          <button onClick={() => setSelectedDepts(departments.map(d => d.id))} className="text-blue-600 hover:underline">All</button>
          <button onClick={() => setSelectedDepts([])} className="text-blue-600 hover:underline">None</button>
          <button onClick={() => {
            const allIds = departments.map(d => d.id);
            setSelectedDepts(allIds.filter(id => !selectedDepts.includes(id)));
